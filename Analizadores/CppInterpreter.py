...
class Interpreter(Visitor):
    ...
    def interpret(self, node):
        ...
        for name, cval in stdlibFunctions.items():
            self.check_env[name] = cval
            self.env[name] = cval
        
        for name, func in stdlibFunctions.items():
    ...
    def visit(self, node: CallExpr):
        ...
        if callee.arity != -1 and len(args) != callee.arity:
            self.error(node.func, f'Número incorrecto de argumentos. Se esperaban {callee.arity} argumentos')
